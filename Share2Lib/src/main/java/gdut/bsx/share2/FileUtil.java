...
public class FileUtil {
    ...
    public static ArrayList<Uri> getFileUriList(Context context, List<String> files){
        ArrayList<Uri> uriList = new ArrayList<>(files.size());

        for (String file: files
             ) {
            if(isImageFile(file)){ //获取图片uri
                uriList.add(getFileUri(context, ShareContentType.IMAGE, new File(file)));
            }else{ //获取非图片(文件)uri
                uriList.add(getFileUri(context, ShareContentType.FILE, new File(file)));
            }
        }
