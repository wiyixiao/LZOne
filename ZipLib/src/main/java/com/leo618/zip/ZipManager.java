...
public final class ZipManager {
    ...
    private static final int     WHAT_ERROR_FINISH = 103;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case WHAT_ERROR_FINISH:
                    ((IZipCallback) msg.obj).onFinish(false);
                    ZipLog.debug("onFinish: success=false");
                    break;
            ...
            public void run() {
                ...
                switch (progressMonitor.getResult()){
                    case ProgressMonitor.RESULT_SUCCESS:
                        timerMsgEnd(WHAT_FINISH);
                        break;
                    case ProgressMonitor.RESULT_ERROR:
                        timerMsgEnd(WHAT_ERROR_FINISH);
                        break;
                }
            }

            private void timerMsgEnd(int state){
                mUIHandler.obtainMessage(state, callback).sendToTarget();
                this.cancel();
                timer.purge();
            }
        }, 0, 300);
